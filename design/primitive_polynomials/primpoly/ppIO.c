        ...
        if (input_arg_string[0] == '-' && input_arg_string[1] != '\0') {
            ...
            for (option_ptr = input_arg_string + 1; *option_ptr != '\0';
                ...
                switch (*option_ptr) {
                    ...
                    case 't':*testPolynomialForPrimitivity = YES;
                        break;

                        /* List all primitive polynomials.  */
                    case 'a':*listAllPrimitivePolynomials = YES;
                        break;

                        /* Print statistics on program operation. */
                    case 's':*printStatistics = YES;
                        break;

                        /* Print help. */
                    case 'h':
                    case 'H':*printHelp = YES;
                        break;

                        /* Turn on all self-checking (slow!).  */
                    case 'c': *selfCheck = YES;
                        break;

                    default:printf("Cannot recognize the option %c\n", *option_ptr);
                        break;
                }
            }

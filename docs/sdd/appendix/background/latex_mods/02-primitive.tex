...
\cite{primitive} 
    ...
    % \subsubsection{Analytical Approach} If the sum of the coefficients of the
    % polynomial equals 1 when $x=0$ and $x=1$, the polynomial is irreducible.
    ...
    %     \centerline{$\therefore$ \examplepoly~ has no roots in $GF(2)[x]$.}

    %     Since the sum of the coefficients is 1 for both the cases, the
    %     polynomial is therefore determined irreducible.

    % \subsubsection{Digital Logic} To design such algorithm, the polynomial may
    % be checked for the following attributes:
    % \begin{enumerate}
